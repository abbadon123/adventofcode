(ns adventofcode-2019.day3.day-3
  (:require [clojure.test :refer :all]))

(defn path [p]
  (let [text (name p)
        goes (-> text first str keyword)
        size (->> text rest (apply str) Integer/parseInt)]
    {:goes goes
     :size size}))

(defn cable [data] (map path data))

(defn move [start times move-fn]
  (->>
    start
    (iterate move-fn)
    (take (inc times))
...
(defn move-right [start times]
  (let [right (fn [[x y]] [(inc x) y])]
    (move start times right)))

(defn move-left [start times]
  (let [left (fn [[x y]] [(dec x) y])]
    (move start times left)))

(defn move-up [start times]
  (let [up (fn [[x y]] [x (inc y)])]
    (move start times up)))

(defn move-down [start times]
  (let [down (fn [[x y]] [x (dec y)])]
    (move start times down)))
  ...
  (reduce
    (fn [acc path]
      (let [point (last acc)
            ...
            size (:size path)
            new (case goes
                  :R (move-right point size)
                  :D (move-down point size)
                  :L (move-left point size)
                  :U (move-up point size)
                  )]
        (concat acc new)))
    [[0 0]]
    cable)
...
(defn intersections [draw1 draw2]
  (let [intersect (vec (clojure.set/intersection (set draw1) (set draw2)))
        not-center (fn [p] (not= p [0 0]))]
    (filter not-center intersect)
    ))

(defn closest [points]
  (let [distance (fn [[x y]] (+ (Math/abs x) (Math/abs y)))
        distances (map distance points)]
    (apply min distances)))

(defn manhattan-distance [data1 data2]
  (let [cable1 (cable data1)
        cable2 (cable data2)
        intersections (intersections (draw cable1) (draw cable2))]
    (closest intersections)))

(defn ascii [draw]
  (let [xs (map first draw)
        ys (map second draw)
        min-x (apply min xs)
        max-x (apply max xs)
        ...
        min-y (apply min ys)
        max-y (apply max ys)
        y-size (+ 10 (- max-y min-y))
        printable (for [y (take y-size (iterate inc min-y))]
                    (for [x (take x-size (iterate inc min-x))] (if (contains? (set draw) [x y]) "0" "."))
                    )
        ]
    (doseq [i printable] (println i))))

(deftest a-test
  (testing "path"
    (is (= (path `R75) {:goes :R :size 75}))
    )
  (testing "cable"
    (is (= (cable `[R75, D30]) [{:goes :R :size 75} {:goes :D :size 30}]))
    )
  (testing "draw"
    (is (= (draw (cable `[R01, D01, L01, U02])) [[0 0] [1 0] [1 -1] [0 -1] [0 0] [0 1]]))
    )
  (testing "intersections"
    (is (= (intersections [[0 0] [0 1] [0 2]] [[0 0] [1 0] [1 1] [0 1] [0 2]]) [[0 2] [0 1]]))
    )
  (testing "closest"
    (is (= (closest [[0 2] [0 1]]) 1)))
  (testing "distance"
    (is (= (manhattan-distance `[R8, U5, L5, D3] `[U7, R6, D4, L4]) 6))
    )
  )
(run-tests)

(time (manhattan-distance
...
(defn steps [data1 data2]
  (let [cable1 (cable data1)
        cable2 (cable data2)
        draw1 (draw cable1)
        draw2 (draw cable2)
        intersections (intersections draw1 draw2)
        steps-draw-1 (fn [intersection] (count (take-while (comp not #{intersection}) draw1)))
        steps-draw-2 (fn [intersection] (count (take-while (comp not #{intersection}) draw2)))
        steps (fn [intersection]
                {:steps        (+ (steps-draw-1 intersection) (steps-draw-2 intersection))
                 :intersection intersection})]
    (apply min-key :steps (map steps intersections))))

(steps `[R8, U5, L5, D3] `[U7, R6, D4, L4])
(steps `[R75, D30, R83, U83, L12, D49, R71, U7, L72]
       `[U62, R66, U55, R34, D71, R55, D58, R83])

(manhattan-distance `[R75, D30, R83, U83, L12, D49, R71, U7, L72]
                    `[U62, R66, U55, R34, D71, R55, D58, R83])
